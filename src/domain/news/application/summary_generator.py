...
@dataclass
class SummaryGenerationResponseDto:
    summary: str
...
class SummaryGeneratorLLM:
    ...
    def generate_summary(self, content: str) -> SummaryGenerationResponseDto:
        messages = fill_message_placeholders(self.msg, placeholders={"article_content": content})
        resp = self.llm.predict_json(messages)
        return SummaryGenerationResponseDto(resp["summary"], resp["one_sentence_summary"])
